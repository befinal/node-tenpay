...
var API = function(config) {
	...
	this.spbill_create_ip = config.spbill_create_ip || '192.168.0.1';
	return this;
}
...
API.prototype.unifiedorder = function(params, callback) {
	...
	var needs = ['body', 'out_trade_no', 'total_fee', 'spbill_create_ip', 'notify_url', 'trade_type'];
	if (pkg.trade_type == 'JSAPI') {
		needs.push('openid');
	} else if (pkg.trade_type == 'NATIVE') {
		needs.push('product_id');
	}
	return this._request(pkg, {type: 'unifiedorder', needs}, callback);
};
...
API.prototype.orderquery = function(params, callback) {
	...
	return this._request(pkg, {type: 'orderquery', needs}, callback);
};
...
API.prototype.closeorder = function(params, callback) {
	...
	return this._request(pkg, {type: 'closeorder', needs}, callback);
};
...
API.prototype.refund = function(params, callback) {
	...
	var cert = true;
	return this._request(pkg, {type: 'refund', needs, cert}, callback);
};
...
API.prototype.refundquery = function(params, callback) {
	...
	return this._request(pkg, {type: 'refundquery', needs}, callback);
};
...
API.prototype.downloadbill = function(params, callback) {
	...
	return this._request(pkg, {type: 'downloadbill', needs}, callback);
};
...
API.prototype.transfers = function(params, callback) {
	...
	var cert = true;
	return this._request(pkg, {type: 'transfers', needs, cert}, callback);
};
...
API.prototype.middleware = function() {
	...
	return function *(next) {
		...
		this.weixin = yield that._validate(xml);
		yield next;
	};
};
// 支付结果通知forExpress
...
// make callback or promise function
API.prototype._request = function(params, options, callback) {
	var that = this;

	// callback
	if (typeof callback == 'function') return that._doRequest(params, options, callback);

	// promise
	return new Promise(function(resolve, reject) {
		that._doRequest(params, options, function(err, result) {
			err ? reject(err) : resolve(result);
		});
	});
};

API.prototype._doRequest = function(params, options, callback) {
	var that = this;
...
API.prototype._validate = function(xml, callback) {
	...
	// callback
	if (typeof callback !== 'function') return that._validateCallback(xml, callback);

	// promise
	return new Promise(function(resolve, reject) {
		that._validateCallback(xml, function(err, result) {
			err ? reject(err) : resolve(result);
		});
	});
}

API.prototype._validateCallback = function(xml, callback) {
	var that = this;
