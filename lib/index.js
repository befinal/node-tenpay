...
class Payment {
  constructor({appid, mchid, partnerKey, pfx, notify_url, refund_url, spbill_create_ip, sandbox} = {}, debug = false) {
    ...
    this.sandbox = sandbox ? '/sandboxnew' : '';
    this.debug = debug;
  }
  ...
  async _parse(xml, type) {
    ...
    if (~['downloadbill', 'downloadfundflow'].indexOf(type)) return xml;
  ...
  async _request(params, type, cert = false) {
    ...
    let {status, data} = await urllib.request(host + this.sandbox + URLS[type], pkg);
  ...
  async getSignkey() {
    ...
    return await this._request(pkg, 'getsignkey');
  }
