...
class Payment {
  ...
  async _parse(xml, type) {
    ...
    switch (type) {
      case 'middleware_nativePay':
        break;
      default:
        if (json.return_code !== 'SUCCESS') throw buildError(json.return_msg ,json.return_msg || 'XMLDataError');
    }
    ...
    switch (type) {
      ...
      case 'getsignkey':
        break;
      default:
        if (json.result_code !== 'SUCCESS') throw buildError(json.err_code_des ,json.err_code || 'XMLDataError');
    }
    ...
    switch (type) {
      case 'getsignkey':
        break;
      case 'middleware_refund':
        if (json.appid !== this.appid) throw buildError('appid不匹配');
        if (json.mch_id !== this.mchid) throw buildError('mch_id不匹配');
        ...
        break;
      case 'transfers':
        if (json.mchid !== this.mchid) throw buildError('mchid不匹配');
        break;
      case 'sendredpack':
      case 'sendgroupredpack':
        if (json.wxappid !== this.appid) throw buildError('wxappid不匹配');
        if (json.mch_id !== this.mchid) throw buildError('mchid不匹配');
        break;
      case 'gethbinfo':
      ...
      default:
        if (json.appid !== this.appid) throw buildError('appid不匹配');
        if (json.mch_id !== this.mchid) throw buildError('mch_id不匹配');
        if (json.sign !== this._getSign(json, json.sign_type)) throw buildError('sign签名错误');
    }
    return json;
  ...
  async _parseBill(xml, format = false) {
    ...
    if (util.checkXML(xml)) {
      ...
      throw buildError(json.err_code_des||json.return_msg ,json.err_code || json.error_code || json.return_msg || 'XMLDataError');
    }
