...
class Payment {
  ...
  async _request(params, type, cert = false) {
    ...
    const request_url = params.request_url || (this.prefix + this.urls[type]);
    delete params.request_url;
    // 安全签名
    ...
    this.log('post data =>\r\n%s\r\n', pkg.data);
    let {status, data} = await urllib.request(request_url, pkg);
  ...
  getPublicKey(params) {
    let pkg = {
      ...
      request_url: 'https://fraud.mch.weixin.qq.com/risk/getpublickey'
    };
  ...
  async payBank(params) {
    ...
    let pkg = {
      ...
      mch_id: this.mchid,
      nonce_str: util.generate(),
      enc_bank_no: util.RSAEncrypt(pub_key, params.enc_bank_no),
      enc_true_name: util.RSAEncrypt(pub_key, params.enc_true_name)
    };
