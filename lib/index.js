...
class Payment {
  ...
  middlewareForExpress(type = 'pay') {
    return async (req, res, next) => {
      ...
      type == 'nativePay' && res.replyNative = prepay_id => {
        res.header('Content-Type', 'application/xml; charset=utf-8');
        res.send(this._getNativeReply(prepay_id));
      }
  ...
  middleware(type = 'pay') {
    return async (ctx, next) => {
      ...
      type == 'nativePay' && ctx.replyNative = prepay_id => {
        ctx.type = 'application/xml; charset=utf-8';
        ctx.body = this._getNativeReply(prepay_id);
      }
