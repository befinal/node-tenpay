...
class Payment {
  ...
  async payBank(params) {
    let pubKey = await this.getPublicKey(params);
    let pkg = {
      ...params,
      mch_id: this.mchid,
      nonce_str: util.generate(),
      enc_bank_no: util.encryptRSA(pubKey, params.enc_bank_no),
      enc_true_name: util.encryptRSA(pubKey, params.enc_true_name)
    };
