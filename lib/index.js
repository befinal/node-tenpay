...
class TenPayError extends Error {
  constructor (message, code) {
    super(message)
    Error.captureStackTrace(this, TenPayError)
    this.code = code
  }
}
  ...
  async _parse(xml, type) {
    ...
    switch (type) {
      case 'middleware_nativePay':
        break;
      default:
        if (json.return_code === 'FAIL') throw new Error(json.return_msg || 'Unknown Return');
    }
    ...
    switch (type) {
      ...
      case 'getsignkey':
        break;
      default:
        if (json.result_code === 'FAIL') throw new TenPayError(json.err_code_des || 'Unknown Return', json.err_code);
    }
